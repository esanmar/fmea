...
class suppliers extends DbTable
{
	...
	public function __construct()
	{
		...
		// supplier
		$this->supplier = new DbField('suppliers', 'suppliers', 'x_supplier', 'supplier', '`supplier`', '`supplier`', 200, 255, -1, FALSE, '`supplier`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->supplier->IsPrimaryKey = TRUE; // Primary key field
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('supplier', $rs))
				AddFilter($where, QuotedName('supplier', $this->Dbid) . '=' . QuotedValue($rs['supplier'], $this->supplier->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('supplier', $row) ? $row['supplier'] : NULL;
		else
			$val = $this->supplier->OldValue !== NULL ? $this->supplier->OldValue : $this->supplier->CurrentValue;
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@supplier@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		$json = "";
		$json .= "supplier:" . JsonEncode($this->supplier->CurrentValue, "string");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->supplier->CurrentValue != NULL) {
			$url .= "supplier=" . urlencode($this->supplier->CurrentValue);
		} else {
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->supplier->CurrentValue = $key;
			else
				$this->supplier->OldValue = $key;
