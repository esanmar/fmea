...
/**
 * Table class for causes
 */
class causes extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'causes';
		$this->TableName = 'causes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`causes`";
		$this->Dbid = 'DB';
		...
		// idCause
		$this->idCause = new DbField('causes', 'causes', 'x_idCause', 'idCause', '`idCause`', '`idCause`', 200, 50, -1, FALSE, '`idCause`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->idCause->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['idCause'] = &$this->idCause;

		// cause
		$this->cause = new DbField('causes', 'causes', 'x_cause', 'cause', '`cause`', '`cause`', 200, 255, -1, FALSE, '`cause`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->cause->Nullable = FALSE; // NOT NULL field
		$this->cause->Required = TRUE; // Required field
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`causes`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "causesedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "causesadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("causesview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("causesview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "causesadd.php?" . $this->getUrlParm($parm);
		else
			$url = "causesadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("causesedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("causesadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("causesdelete.php", $this->getUrlParm());
	}
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->idCause->CurrentValue = $key;
			else
				$this->idCause->OldValue = $key;
	...
	public function renderEditRow()
	{
		...
		$this->idCause->EditValue = $this->idCause->CurrentValue;
